...
export const slowAES = {
	...
	aes: {
		...
		expandKey: function (key, size) {
			...
			const expandedKey = [];
			for (let i = 0; i < expandedKeySize; i++) expandedKey[i] = 0;

			/* set the 16,24,32 bytes of the expanded key to the input key */
			for (let j = 0; j < size; j++) expandedKey[j] = key[j];
			currentSize += size;
			...
			while (currentSize < expandedKeySize) {
				...
				if (size == this.keySize.SIZE_256 && currentSize % size == 16)
					for (let l = 0; l < 4; l++) t[l] = this.sbox[t[l]];
		...
		mixColumns: function (state, isInv) {
			...
			for (let i = 0; i < 4; i++) {
				...
				for (let j = 0; j < 4; j++) column[j] = state[j * 4 + i];
				/* apply the mixColumn on one column */
				column = this.mixColumn(column, isInv);
				/* put the values back into the state */
				for (let k = 0; k < 4; k++) state[k * 4 + i] = column[k];
			}
			return state;
		},
		...
		mixColumn: function (column, isInv) {
			...
			const cpy = [];
			for (let i = 0; i < 4; i++) cpy[i] = column[i];
		...
		encrypt: function (input, key, size) {
			...
			const expandedKey = this.expandKey(
				key,
				size,
			); /* the expanded key */
